...
{
	let ratio = window.innerWidth*1.21 / window.innerHeight;

	if (ratio > 1)
	{
		grid_size_x = initial_max_grid_size - 1;
...
function generate_grid()
{
	...
	start_pos = [parseInt(document.getElementById("start_x").value),parseInt(document.getElementById("start_y").value)];
	target_pos = [grid_size_x - 2, grid_size_y - 2]
...
{
	return document.getElementById(".x_" + x.toString(10) + ".y_" + y.toString(10));
}
...
{
	document.getElementById("#my_table").addEventListener('mousedown', event =>
	{
		event.preventDefault();
		clicking = true;
